...
import { getUserByEmail, isAdmin } from '../lib/supabase';
...
const Login: React.FC<LoginProps> = ({ onLogin, onAdminLogin, onBack, onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mode admin ou utilisateur normal
      const user = await getUserByEmail(emailTrimmed);
      
      if (!user) {
        setError('Aucun compte trouvé avec cet email. Votre compte est créé automatiquement lors de votre première commande.');
        setLoading(false);
        return;
      }
      ...
      if (isAdminMode) {
        ...
        // Pour l'admin, utiliser onAdminLogin
        await onAdminLogin(emailTrimmed);
        onNavigate('admin');
        return;
      }

      // Si l'utilisateur est admin mais n'a pas activé le mode admin, le rediriger vers admin
      if (userIsAdmin) {
        await onAdminLogin(emailTrimmed);
        onNavigate('admin');
        return;
      ...
      // Utilisateur normal trouvé, le connecter
      onLogin(user);
